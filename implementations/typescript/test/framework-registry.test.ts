...
test("Frappe keeps the missing Bench runtime proof explicit", () => {
  const frappe = getFrameworkAdapter("frappe");
  assert.ok(frappe);
  assert.equal(frappe.layers.transaction_adapter.status, "implemented");
  assert.equal(frappe.layers.behavioral_runtime_lab.status, "partial");
  assert.equal(frappe.layers.behavioral_runtime_lab.proof, "contract");

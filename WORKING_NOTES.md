...
- hardened Frappe whitelist attribution so only the exact decorated function receives an entrypoint role; neighboring source text cannot leak whitelist semantics.
...
- Expand Frappe framework resolution for imported alias/name resolution, multiline/dynamic decorator attribution, custom or indirect DocType controller resolution and other dynamic composition that can be proven without optimistic inference.
